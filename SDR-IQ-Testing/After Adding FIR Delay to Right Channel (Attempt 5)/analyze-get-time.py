...
def measure_delay_and_phase(left, right, freq_hz):
    ...
    # Find the bin with the strongest signal (the test tone)
    idx = np.argmax(np.abs(fft_l))
    
    phase_l = np.angle(fft_l[idx])
    phase_r = np.angle(fft_r[idx])
    
    # Calculate signed phase difference
    ...
    # Convert phase to time delay in seconds: t = phase / (2 * pi * f)
...
def analyze_adc_with_corrections(directory, delay_us, gain_db, num_taps):
    ...
    for file in files:
        ...
        _, phase_fixed = measure_delay_and_phase(left, right_fixed, freq_hz)
        
        results.append({
            'freq': freq_hz,
            'orig_sd_ratio': sd_ratio_orig, 'orig_lr_imb': lr_imb_orig, 'orig_phase': abs(phase_orig),
            'fixed_sd_ratio': sd_ratio_fixed, 'fixed_lr_imb': lr_imb_fixed, 'fixed_phase': abs(phase_fixed)
        })
