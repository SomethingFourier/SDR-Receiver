...
static void udp_recv_callback(void *arg, struct udp_pcb *pcb, struct pbuf *p, const ip_addr_t *addr, u16_t port) {
  ...
  // Assuming ASCII payload, null terminate it for safe string operations
  char buffer[64];
  u16_t len = p->tot_len;
  ...
  // Check if it's a TUNE command
  if (strncmp(buffer, "TUNE ", 5) == 0) {
      long int freq = strtol(buffer + 5, NULL, 10);
      printf("UDP Control: Requested Tune Frequency: %ld Hz\n", freq);
      
      // The RP2350 determines the closest frequency it can tune to.
      // For now, we simulate this by just taking the requested frequency.
      long int actual_freq = freq;
      printf("UDP Control: Actual Tune Frequency: %ld Hz\n", actual_freq);

      // Send response back
      char response[64];
      int resp_len = snprintf(response, sizeof(response), "TUNED %ld\n", actual_freq);
      
      struct pbuf *p_resp = pbuf_alloc(PBUF_TRANSPORT, resp_len, PBUF_RAM);
      if (p_resp != NULL) {
          pbuf_take(p_resp, response, resp_len);
...
int main() {
  ...
  // Setup UDP Control Server
  struct udp_pcb *upcb = udp_new();
  if (upcb != NULL) {
    udp_bind(upcb, IP_ADDR_ANY, 50000);
    udp_recv(upcb, udp_recv_callback, NULL);
    printf("UDP Control server listening on port 50000\n");
  }
