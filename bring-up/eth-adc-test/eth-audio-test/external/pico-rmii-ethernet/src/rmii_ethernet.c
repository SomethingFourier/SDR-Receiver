...
volatile uint32_t tx_rd_pkt_ptr = 0;
...
static err_t netif_rmii_ethernet_output(struct netif *netif, struct pbuf *p) {
  ...
  uint32_t tx_free = TX_BUF_SIZE - ((curr_wr - curr_rd) & TX_BUF_MASK);
  ...
  tx_curr_pkt_ptr = (tx_curr_pkt_ptr + 1) & TX_NUM_MASK;
  
  // Start DMA immediately if it is idle
...
static err_t netif_rmii_ethernet_low_init(struct netif *netif) {
  ...
  // Disable command chaining, trigger manually instead
  channel_config_set_chain_to(&tx_dma_channel_config, tx_dma_chan);
